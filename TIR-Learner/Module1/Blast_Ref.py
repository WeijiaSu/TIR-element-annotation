...
def BlastRef(arglist):
  ...
  #  blast = "blastn -query %s -db %s -num_threads %s -outfmt=7 -out %s" % (path+"/RefLib/"+refLib , genomedb, int(t), targetDir+genomeName+spliter+"blast"+spliter+refLib) #shujun
    blast = "blastn -max_hsps 5 -perc_identity 80 -qcov_hsp_perc 100 -query %s -db %s -num_threads %s -outfmt '7 qacc sacc length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s" % (path+"/RefLib/"+refLib , genomedb, int(t), targetDir+genomeName+spliter+"blast"+spliter+refLib) #shujun
