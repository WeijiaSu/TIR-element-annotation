...
def BlastRef(file):
    ...
    for refLib in ref_list:
        blast = "blastn -max_hsps 5 -perc_identity 80 -qcov_hsp_perc 80 -query %s -subject %s -outfmt '7 qseqid sseqid length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s" % (file , path+"/RefLib/"+refLib, targetDir+file+spliter+"blast"+spliter+refLib)
